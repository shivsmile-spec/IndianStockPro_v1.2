...
(function(){
  ...
  function injectStyle(){
    ...
    style.textContent=`
      .isp-live-price-box{margin-top:2px;text-align:right;line-height:1.2}
      ...
      .isp-live-change{font-size:12px;font-weight:900;margin-top:3px}
      ...
      @media(max-width:700px){.isp-live-price-main{font-size:18px}.isp-live-price-meta{font-size:9px;white-space:normal}}
    `;
  ...
  function renderCard(card,q){
    ...
    el.innerHTML=`<div class="isp-live-price-box"><div class="isp-live-price-main"><span class="isp-live-tag">LIVE</span>${esc(money(price))}</div><div class="isp-live-change ${cls}">${esc(pctText+changeText)}</div><div class="isp-live-price-meta">Prev close ${esc(money(prev))}${updated?` · ${esc(updated)}`:""}</div></div>`;
  }
