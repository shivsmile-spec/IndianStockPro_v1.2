...
(function(){
  ...
  function panel(el){return el?.closest("section.panel")||el||null}
  function mark(id,el,title,desc){
    ...
    if(title&&!p.querySelector(".isp-tab-title")){
      const h=document.createElement("div");h.className="isp-tab-title";
      h.innerHTML=`<strong>${esc(title)}</strong><span>${esc(desc||"")}</span>`;p.insertBefore(h,p.firstChild);
    }
    return p;
  }
  function discover(){
    ...
    const newsEl=document.querySelector(".expert-panel");if(newsEl&&!newsEl.id)newsEl.id="isp-news-context";
    mark("isp-news-context",newsEl,"News & Market Context","External news, industry and broader market intelligence.");
  ...
  function target(id){
    const t=TABS.find(x=>x[0]===id);if(!t)return null;
  ...
  function build(){
    ...
    window.ispActivateTab=activate;window.ispDashboardTabsReady=true;
  }
  ...
  function start(){
    style();discover();build();activate("indices");
    const root=document.querySelector(".container")||document.body;
    new MutationObserver(()=>{clearTimeout(observerTimer);observerTimer=setTimeout(reconcile,120)}).observe(root,{childList:true,subtree:true});
