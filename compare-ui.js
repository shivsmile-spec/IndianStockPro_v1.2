...
(function(){
  ...
  function getSymbols(){
    try{return JSON.parse(sessionStorage.getItem("isp_compare")||"[]")}catch(e){return []}
  }
  function saveSymbols(a){sessionStorage.setItem("isp_compare",JSON.stringify(a.slice(0,4)))}

  function removeOldBottomBar(){
    var old=document.getElementById("isp-compare");
    if(old)old.remove();
  }
  ...
  function ensurePanel(){
    ...
    var host=search&&search.closest("section.panel");
    if(!host)return null;
  ...
  function render(){
    ...
    if(!syms.length){
      p.innerHTML='<div class="isp-tc-head"><div><div class="isp-tc-title">⚖ Compare Stocks</div><div class="isp-tc-sub">Select stocks from their analysis reports to compare up to 4 stocks.</div></div></div><div class="isp-tc-empty">No stocks selected yet. Open a stock analysis and press <b>⚖ Compare</b>.</div>';
      return
    }
