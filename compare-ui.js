...
(function(){
  ...
  var positiveNum=function(v){var n=num(v);return n!=null&&n>0?n:null};
    ...
    loading=Promise.all(DATA.map(function(u){return fetch(u+"?v="+Date.now(),{cache:"no-store"}).then(function(r){if(!r.ok)throw Error(u+" HTTP "+r.status);return r.json()})})).then(function(a){
      ...
      db.stocks=crows.filter(function(x){return x&&x.symbol}).map(function(x){
        ...
        return {
          ...
          price:positiveNum(x.price)!=null?positiveNum(x.price):positiveNum(q.price),
  ...
  function liveHtml(x){
    var q=x.quote,p=positiveNum(q&&q.price),pct=num(q&&q.changePct),prev=positiveNum(q&&q.previousClose);
    if(p==null)return '<div class="isp-c-research">'+money(x.price)+'</div><div class="isp-c-live-meta">Research price · Published live quote unavailable</div>';
