...
(function(){
  ...
  async function getJson(path){var r=await fetch(path+"?v="+Date.now(),{cache:"no-store"});if(!r.ok)throw Error(path+" HTTP "+r.status);return r.json()}
  function asRows(r){
    if(Array.isArray(r))return r;
    if(r&&Array.isArray(r.stocks))return r.stocks;
    if(r&&Array.isArray(r.analyses))return r.analyses;
    if(r&&Array.isArray(r.results))return r.results;
    if(r&&r.stocks&&typeof r.stocks==='object')return Object.keys(r.stocks).map(function(k){var x=r.stocks[k]||{};if(!x.symbol)x.symbol=k;return x});
    if(r&&r.analyses&&typeof r.analyses==='object')return Object.keys(r.analyses).map(function(k){var x=r.analyses[k]||{};if(!x.symbol)x.symbol=k;return x});
    if(r&&r.data&&typeof r.data==='object')return Object.keys(r.data).map(function(k){var x=r.data[k]||{};if(x&&typeof x==='object'&&!x.symbol)x.symbol=k;return x}).filter(function(x){return x&&x.symbol});
    return [];
  }
  async function loadData(){
    if(loaded)return;
    if(loading)return loading;
    loading=Promise.all([getJson("./data/nse_universe.json"),getJson("./data/search_analysis.json"),getJson("./data/live_quotes.json"),getJson("./data/fundamentals.json")]).then(function(a){
      var u=a[0],r=a[1],q=a[2],f=a[3];
      universe=Array.isArray(u)?u:(u&&Array.isArray(u.stocks)?u.stocks:[]);
      researchBy={};asRows(r).forEach(function(x){if(x&&x.symbol)researchBy[norm(x.symbol)]=x});
      if(Array.isArray(window.integrated))window.integrated.forEach(function(x){if(x&&x.symbol)researchBy[norm(x.symbol)]=x});
      quotes=(q&&q.quotes)||{};
      fundamentalsBy={};var fs=Array.isArray(f)?f:(f&&Array.isArray(f.stocks)?f.stocks:[]);
      if(Array.isArray(f))fs.forEach(function(x){if(x&&x.symbol)fundamentalsBy[norm(x.symbol)]=x});
      else if(f&&f.stocks&&typeof f.stocks==='object')fundamentalsBy=f.stocks;
      loaded=true;
  ...
  function stocks(){
    ...
    Object.keys(researchBy).forEach(function(s){if(!seen[s]){seen[s]=1;out.push({symbol:s,name:researchBy[s].company||researchBy[s].companyName||s,series:researchBy[s].series||"EQ"})}});
  ...
  function companyName(sym){var r=findResearch(sym),u=universe.find(function(x){return norm(x.symbol)===norm(sym)}),f=findFund(sym);return (r&&(r.company||r.companyName))||(u&&(u.companyName||u.company))||(f&&(f.companyName||f.company))||sym}
  function css(){if(document.getElementById("isp-trial-compare-css"))return;var s=document.createElement("style");s.id="isp-trial-compare-css";s.textContent=`#isp-trial-compare-panel{margin-top:14px;border:1px solid #dfe5ee;border-radius:14px;background:#fff;padding:16px;box-shadow:0 4px 18px rgba(0,0,0,.05)}.isp-tc-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap}.isp-tc-title{font-size:18px;font-weight:900;color:#172033}.isp-tc-sub{font-size:11px;color:#667085;margin-top:4px}.isp-tc-actions{display:flex;gap:7px;flex-wrap:wrap}.isp-tc-btn{border:1px solid #ccd3df;background:#fff;color:#172033;border-radius:9px;padding:7px 10px;font-size:12px;font-weight:800;cursor:pointer}.isp-tc-btn.primary{background:#10182b;color:#fff;border-color:#10182b}.isp-tc-selectors{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.isp-tc-selector{border:1px solid #e1e6ef;border-radius:11px;padding:11px;background:#f8fafc}.isp-tc-selector label{display:block;font-size:11px;font-weight:800;color:#667085;margin-bottom:6px}.isp-tc-search{position:relative}.isp-tc-search input{width:100%;padding:9px 11px;border:1px solid #ccd3df;border-radius:8px;background:#fff;color:#172033;font-weight:600;outline:none;font-size:13px}.isp-tc-results-list{position:absolute;left:0;right:0;top:calc(100% + 4px);z-index:60;max-height:220px;overflow:auto;background:#fff;border:1px solid #ccd3df;border-radius:9px;box-shadow:0 8px 22px rgba(16,24,43,.14);display:none}.isp-tc-results-list.open{display:block}.isp-tc-option{padding:9px 11px;cursor:pointer;border-bottom:1px solid #eef1f5;font-size:12px}.isp-tc-option:hover{background:#f5f7fa}.isp-tc-option b{font-weight:900}.isp-tc-option span{color:#667085;margin-left:5px}.isp-tc-no-results,.isp-tc-empty,.isp-tc-loading{padding:10px 11px;color:#667085;font-size:12px;background:#f7f9fc;border-radius:10px;margin-top:12px}.isp-tc-hint{margin-top:10px;font-size:11px;color:#667085}.isp-tc-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:14px}.isp-tc-card{border:1px solid #e1e6ef;border-radius:12px;padding:12px;background:#fff}.isp-tc-symbol{font-size:16px;font-weight:900}.isp-tc-name{font-size:11px;color:#667085;margin-top:3px;min-height:28px}.isp-tc-row{display:flex;justify-content:space-between;gap:8px;border-top:1px solid #eef1f5;padding-top:7px;margin-top:7px;font-size:11px}.isp-tc-label{color:#667085}.isp-tc-value{font-weight:900;text-align:right}.isp-tc-note{margin-top:9px;padding:8px;border-radius:8px;background:#f7f9fc;color:#667085;font-size:10px;line-height:1.4}.isp-tc-remove{margin-top:10px;width:100%;border:1px solid #e1e6ef;background:#f8fafc;border-radius:8px;padding:6px;font-size:10px;font-weight:800;cursor:pointer;color:#596273}@media(max-width:900px){.isp-tc-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.isp-tc-selectors,.isp-tc-grid{grid-template-columns:1fr}}`;document.head.appendChild(s)}
  ...
  function card(sym){
    var r=findResearch(sym),q=findQuote(sym),f=findFund(sym),quant=(r&&typeof r.quantitative==='object'&&r.quantitative)||{};
    var price=q&&q.price!=null?q.price:(r&&r.liveQuote&&r.liveQuote.price!=null?r.liveQuote.price:(r&&r.price!=null?r.price:null));
    var quantScore=quant.score!=null?quant.score:(r&&r.quantitativeScore!=null?r.quantitativeScore:(r&&r.score!=null?r.score:null));
    var confidence=quant.confidence!=null?quant.confidence:(r&&r.confidence!=null?r.confidence:null);
    var signal=quant.signal||(r&&r.signal)||'';var risk=quant.risk!=null?quant.risk:(r&&r.risk!=null?r.risk:null);
    var fundScore=f&&(f.score!=null?f.score:f.healthScore),health=f&&(f.health||f.status);var note=[];
    if(quantScore==null)note.push('Quantitative score not available in the published analysis snapshot.');
    if(!q&&!(r&&r.liveQuote))note.push('No published live quote is currently available for this stock in the quote feed.');
  ...
  function renderCards(selected){if(!selected.length)return '<div class="isp-tc-empty">No stocks selected yet. Search and select 2–4 stocks above.</div>';return '<div class="isp-tc-grid">'+selected.map(card).join('')+'</div>'}
  function render(p){
    var list=stocks(),selected=getSelected();
    p.innerHTML='<div class="isp-tc-head"><div><div class="isp-tc-title">⚖ Compare Stocks</div><div class="isp-tc-sub">Search and select 2–4 NSE stocks for a side-by-side comparison.</div></div><div class="isp-tc-actions"><button type="button" class="isp-tc-btn" id="ispTcClear">Clear</button><button type="button" class="isp-tc-btn" id="ispTcClose">Close</button></div></div><div class="isp-tc-selectors">'+[0,1,2,3].map(function(i){return '<div class="isp-tc-selector"><label>Stock '+(i+1)+(i<2?' *':' optional')+'</label><div class="isp-tc-search"><input type="text" autocomplete="off" data-tc-input="'+i+'" placeholder="Search symbol or company name" value="'+esc(selected[i]||'')+'"><div class="isp-tc-results-list" data-tc-list="'+i+'"></div></div></div>'}).join('')+'</div><div class="isp-tc-actions" style="margin-top:12px"><button type="button" class="isp-tc-btn primary" id="ispTcCompare">Compare selected</button></div><div id="isp-tc-results">'+renderCards(selected)+'</div><div class="isp-tc-hint">Type a symbol or company name; matching NSE choices appear below.</div>';
    function closeLists(except){p.querySelectorAll('.isp-tc-results-list').forEach(function(x){if(x!==except)x.classList.remove('open')})}
    function choose(i,sym){var a=getSelected();a[i]=sym;saveSelected(a);render(p)}
    ...
    p.onclick=function(e){if(!e.target.closest('.isp-tc-search'))closeLists()};p.querySelector('#ispTcClear').onclick=function(){saveSelected([]);render(p)};p.querySelector('#ispTcClose').onclick=function(){p.remove()};p.querySelectorAll('[data-tc-remove]').forEach(function(b){b.onclick=function(){saveSelected(getSelected().filter(function(x){return x!==b.dataset.tcRemove}));render(p)}});p.querySelector('#ispTcCompare').onclick=function(){var a=getSelected();if(a.length<2){p.querySelector('#isp-tc-results').innerHTML='<div class="isp-tc-empty">Please select at least <b>2 stocks</b> to compare.</div>';return}p.querySelector('#isp-tc-results').scrollIntoView({behavior:'smooth',block:'nearest'})};
  }
